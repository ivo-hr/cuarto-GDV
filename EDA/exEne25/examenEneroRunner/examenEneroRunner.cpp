...
Arreglo: 3er unordered_map para los equipos jugados sin que se repitan
Arreglo: 4º unordered_map
*/
...
public:
    // Coste: O(n) en el peor caso (el jugador existía en otro equipo), n siendo el número de jugadores del equipo anterior.
    ...
    // Coste: O(n), n siendo el menor entre el número de jugadoers que hay que devolver y el numero de jugadores en un equipo
    list<Jugador> ultimosFichajes(const Equipo& equipo, int n) const {
        list<Jugador> toRet;
        auto itEquipos = fichajesEquipo.find(equipo);
        if (itEquipos == fichajesEquipo.end())
            throw domain_error("Equipo inexistente");
        //Si no hay jugadores, devolvemos la lista vacía (supongo?)
        else if (itEquipos->second.empty())
            return toRet;
        //list<Jugador> toRet;
        //Coje los ultimos fichajes en orden de llegada
