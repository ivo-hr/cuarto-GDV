...
public:
    ...
    Set<T>& operator||(Set<T> const& other){
        ...
        //Calculamos el máximo de elementos de los dos conjuntos
        int maxElem = max(nelems, other.nelems);
        //Unimos los dos conjuntos, borrando los repetidos
        //La función add ya se encarga de no añadir repetidos
        for (int i = 0; i < maxElem; i++){
            if (i < nelems){
                toReturn.add(array[i]);
            }
            if (i < other.nelems){
                toReturn.add(other.array[i]);
            }
        }
    }
    ...
    Set<T> operator&&(Set<T> const& other){
        ...
        //Haremos un for con el de menor elementos
